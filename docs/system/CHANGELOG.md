...
## MR-7 — Separate scaffold ownership

New projects separate Git instructions, system knowledge and runtime procedures.
Project knowledge is seeded once and protected from later template upgrades.
Shared Tracker helpers move under `.workflow/tools/` with compatibility wrappers;
the engine pin and evidence semantics stay unchanged. Existing mixed layouts
require an audited migration. See [MR-7](https://github.com/MojoTheApe/master-repo/issues/7).
